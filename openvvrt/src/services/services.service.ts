...
export class ServicesService {
  ...
  public async getAll() {
    ...
    if (stderr) throw new InternalServerErrorException(stderr);
  ...
  public async restart(service: string) {
    ...
    if (stderr) throw new BadRequestException(stderr);
    return s.split(' ');
  }
  ...
  public async start(service: string) {
    ...
    await Aigle.eachSeries(s.split(' '), async (e) => {
      const { stderr } = await execSync(
        `cd ${this.SERVICES_FOLDER_PATH}/${e} && sudo pm2 start -f $(find -wholename './dist/*index.js') --name ${e}`,
      );
      if (stderr) throw new InternalServerErrorException(stderr);
    });
    return s.split(' ');
  ...
  public async build(service: string) {
    ...
    await Aigle.eachSeries(s.split(' '), async (e: string) => {
      const { stderr } = await execSync(
        `cd ${this.SERVICES_FOLDER_PATH}/${e} && sudo rm -rf dist node_modules && npm i && tsc`,
      );
  ...
  public async stop(service: string) {
    const s = service || (await this.getAllServicesNames());
    const { stderr } = await execSync(
      `sudo pm2 stop ${s} && sudo pm2 delete ${s}`,
    );
    if (stderr) throw new InternalServerErrorException(stderr);
    return s.split(' ');
  }
  ...
  public async logs(service: string, lines: string) {
    ...
    const { stdout, stderr } = await execSync(
      `pm2 logs --nostream --lines ${lines} ${service}`,
    );
    if (stderr) throw new InternalServerErrorException(stderr);
    return stdout.split('\n');
  }
